...
"""
        let city = try JSONDecoder().decode(City.self, from: Data(json.utf8))
...
"""
        let body = try JSONDecoder().decode(CityResponseBody.self, from: Data(json.utf8))
        XCTAssertEqual(body.cityList.count, 1)
    }    
}
